...
19a. **Self-service password reset** is tenant-configurable across a catalog of channels, not a single fixed flow: emailed time-limited single-use reset link (default), SMS-based reset code, and **supervisor-assisted reset** (a designated supervisor/admin verifies identity in person and triggers/approves the reset — for field staff without reliable personal device/email access). A Tenant Admin enables whichever channel(s) fit their workforce; more than one may be active simultaneously, and the user picks among the tenant's enabled channels at request time.
19b. Password reset is gated by the tenant's **minimum required AAL** (#7), same bar as login/MFA: a channel whose assurance level falls below the tenant's configured minimum is not offered, regardless of platform-wide availability. A DOE/federal tenant requiring AAL3 cannot reset via a bare emailed link (AAL1) — that tenant is limited to supervisor-assisted reset (with in-person identity verification substituting for a weak automated channel) or a channel paired with an already-enrolled strong second factor. This mirrors item 7's method-eligibility filtering exactly, applied to reset instead of login.
19c. Every reset request and completion (channel used, initiating and approving accounts where applicable) is written to the immutable audit trail, consistent with the platform's other credential-lifecycle events.
...
- [ ] A commercial tenant with email and SMS reset channels enabled lets a user complete a self-service password reset via either channel, choosing at request time.
- [ ] A DOE/federal tenant set to minimum AAL3 does not offer emailed-link password reset at all; only supervisor-assisted reset (or another channel meeting AAL3) is available.
- [ ] Every password reset request and completion is written to the immutable audit trail, including which channel was used and, for supervisor-assisted resets, who approved it.
