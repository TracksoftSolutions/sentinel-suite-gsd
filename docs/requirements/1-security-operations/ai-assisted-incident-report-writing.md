...
1. **Response Timeline Events** — a new thin Activity extension (`origin_incident_ref`, `phase`, `phase_timestamp`, `recorded_by`, optional `source_location_ref`) capturing the classic response lifecycle: call received, dispatched, en route (with where the responding unit started from), arrived, on scene, cleared. Modeled as real Activities per the user's own framing ("they are activities related to the incident"), each phase its own timestamped event rather than a handful of plain fields — the same "everything trackable is an Activity" discipline used throughout. **Superseded (retrofit): [Unit Dispatch & Proximity Routing](../2-dispatch-cad/unit-dispatch-proximity-routing.md)'s Dispatch Activity extension is now the canonical source of the `dispatched`/`en_route`/`arrived`/`on_scene`/`cleared` phases** — see Functional Requirement #1a and Data Model below. This mechanism remains fully live only for the case it was originally built for: a self-initiated Incident with no upstream Call/Dispatch record.
2. **Quick field notes** — ordinary Incident Updates (already established), entered via typed text, **voice-to-text** (the `voice_transcription` AI context this doc establishes against [AI/LLM Services](../0-platform-core/ai-llm-services.md), later reused by CLI-Style Input's own AI-assist mode voice input), or a tap on a **tenant-configurable canned phrase** ("No injuries," "Suspect fled northbound," "Area secured") that logs instantly with zero typing.
3. **AI Draft Narrative** — an explicit, human-triggered action (never automatic) that synthesizes the Incident's Response Timeline Events, Incident Updates, and core structured fields (category, severity, participants) into a coherent draft narrative, via the `incident_narrative` AI context (provider, prompt template, and system instructions all resolved and owned by AI/LLM Services, not this doc). A draft is always a working, editable record — never auto-published. Once a human reviews, edits as needed, and approves it, the final text commits as a normal, immutable **Incident Update** tagged `source = ai_generated`, referencing which timeline data it was synthesized from — keeping "the Update timeline is the complete record" true, and letting Incident Report generation include it exactly like any other update, with no special-casing.
...
1. **Response Timeline Event** registers as a thin Activity extension: `origin_incident_ref` (direct field, fixed at creation, same non-EntityAssociation reasoning as Incident Update's own parent link), `phase` (call_received, dispatched, en_route, arrived, on_scene, cleared — platform-fixed, not tenant-configurable, since these are structural response-lifecycle stages), `phase_timestamp`, `recorded_by`, `source_location_ref` (nullable, meaningful for `en_route` — where the responding unit started from), `source_dispatch_ref` (nullable, retrofit — see #1a).
1a. **Retrofit — canonical source for four of six phases.** When an Incident is escalated from a Call that has a corresponding Dispatch record (Unit Dispatch & Proximity Routing) for the responding officer, that officer's `dispatched`/`en_route`/`arrived`/`on_scene`/`cleared` Response Timeline Events set `source_dispatch_ref` and derive their `phase_timestamp` values from that Dispatch record's own phase-timeline rather than being independently logged a second time. `call_received` is unaffected — it continues sourcing from the originating Call's own `received_at` (Call Intake & Logging). A self-initiated Incident with no upstream Call/Dispatch logs all phases directly exactly as originally specified, `source_dispatch_ref` staying null throughout.
2. A Response Timeline Event with no `source_dispatch_ref` is created either **self-logged** by the responding Guard (a single tap stamps `now()`) or **on behalf of** a unit by a Supervisor/Dispatcher-equivalent — the identical self-or-on-behalf-of posture Patrol Management already established for Mobile Patrol Unit location, including the ability to correct a timestamp after the fact (audit-logged, per Structured Logging & Audit Trails).
...
- source_dispatch_ref (nullable — FK → Dispatch; when set, this phase's timestamp derives from that Dispatch record rather than being independently logged, per retrofit #1a)
- corrected (bool), correction_history[] (prior value, changed_by, changed_at) — if corrected after initial logging; not applicable to a derived (`source_dispatch_ref` set) event, which instead reflects the linked Dispatch record's own correction history
...
- **AI/LLM Services**: owns the actual provider abstraction (SaaS-pooled or BYO API key), Prompt Templates, and category × location Custom Instructions this doc's `incident_narrative` and `voice_transcription` AI contexts resolve against — this doc declares the contexts and their placeholder variables (incident category, severity, timeline events, updates text, location name); it never talks to a provider or assembles a prompt directly.
- **CLI-Style Input**: consumes the `voice_transcription` AI context this doc establishes for its own AI-assist mode's optional voice input, rather than building its own — the dependency direction that doc's own Integrations section already anticipated.
...
- **Unit Dispatch & Proximity Routing (Module 2 Dispatch/CAD)**: as anticipated, now owns the canonical response-phase timeline via its Dispatch Activity extension. Response Timeline Event is retrofitted (`source_dispatch_ref`, #1a) to derive four of its six phases from the linked Dispatch record when one exists, remaining fully independent only for self-initiated incidents with no upstream Call/Dispatch.
- **Call Intake & Logging (Module 2 Dispatch/CAD)**: source of `received_at`, which `call_received`-phase Response Timeline Events (derived or direct) ultimately trace back to for any Incident that originated from a Call.
...
- ~~Whether a dedicated "AI/LLM Services" Platform Core feature is eventually needed~~ — resolved: see [ai-llm-services.md](../0-platform-core/ai-llm-services.md), which now owns provider abstraction (SaaS-pooled or BYO API key across Anthropic/OpenAI/Gemini/Azure OpenAI), Prompt Templates, and category × location Custom Instructions configuration. This doc's `context_key = incident_narrative` and `voice_transcription` declarations, and their placeholder variables, are defined there.
...
- ~~Dispatch/CAD reconciliation~~ — resolved: see [unit-dispatch-proximity-routing.md](../2-dispatch-cad/unit-dispatch-proximity-routing.md), whose Dispatch Activity extension is now canonical for four of six phases via `source_dispatch_ref`.
