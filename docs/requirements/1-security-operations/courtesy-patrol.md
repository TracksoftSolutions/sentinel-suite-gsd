...
**Some categories need a few extra structured fields the others don't** (a Jump Start cares about battery type; a Tire Change cares about which tire; a generic Patrol needs neither). Rather than giving each category its own full Activity-extension TPT level — disproportionate machinery for "a few optional extra fields on a narrative record" — Courtesy Patrol uses the platform's **`extended_fields`** mechanism: a bounded, schema-governed JSONB column physically living on Entity Registry Core's base `Entity` row (shared, per TPT, by every level including this one), governed end-to-end by [Tenant-Defined Types & Custom Fields](../0-platform-core/tenant-defined-types-custom-fields.md). This pattern was drafted locally here first, then **promoted to a first-class Platform Core feature** during elicitation so any future feature with the same "a few optional fields that don't earn a full TPT level" need can reuse one governed mechanism instead of reinventing it. Courtesy Patrol's own contribution is the **carrier subdivision**: its Category Definition registers a finer-grained carrier per tenant-configurable category (`courtesy_patrol:category:<id>`, rather than one carrier for the whole Activity extension type) and declares which small field set applies to it (`extra_fields_schema[]`) — the actual storage and write-time validation belong entirely to the owning platform feature, not reimplemented here.
...
- **Entity Registry Core**: identity, dedup/merge, and display-label requirements, same as any other Activity type; also the physical carrier of the `extended_fields` column this doc's category details ultimately live on.
- **Tenant-Defined Types & Custom Fields**: owns the `extended_fields` governance/validation service itself (promoted here during this doc's elicitation) — Courtesy Patrol supplies the per-category carrier subdivision and schema selector (`extra_fields_schema[]` on its own Category Definition), not the underlying storage or validation.
...
- Whether `extended_fields` field types need richer validation (e.g., enum/select options) beyond the basic key/label/type/required shape — a technical-spec-level decision, shared with Tenant-Defined Types & Custom Fields' own open question on the same point.
