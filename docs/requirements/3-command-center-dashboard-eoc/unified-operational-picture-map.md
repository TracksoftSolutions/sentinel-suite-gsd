...
2. Four layer types compose on the map, each independently toggleable:
   ...
   - **Gate/Barrier Positions and Intercom Positions** *(retrofit, by Remote Gate & Barrier Controls)* — fixed pins for every registered Gate/Barrier Position and Intercom Position, the same triad-pin-layer convention a fourth and fifth time; clicking a Gate/Barrier pin surfaces its Open Barrier action (per that doc's confirmation-gate resolution), clicking an Intercom pin surfaces Answer Intercom — off by default in both day-one presets (#5).
