...
9. At day one: `card` — Call, Incident, Patrol, Route Assignment (rollup-only, no Feed children, per #8), Patrol Request, Patrol Finding, Courtesy Patrol (only when parentless, per #6), Resource Request (retrofit, see EOC Logistics Hub). `feed` — Dispatch (parent: Call), Checkpoint Scan (parent: Patrol), Incident Update (parent: Incident), Courtesy Patrol (parent: Patrol Request, when set), Agency Handoff Log (parent: Incident — retrofit, see Multi-Incident Console). `none` — Daily Activity Report entries, and every other registered type by default until its owning module explicitly registers it otherwise.
...
- **Genuine transitive (2+ level) Feed nesting remains unsolved for this doc's own live board.** Guard Tour's real chain is Route Assignment → Patrol → Checkpoint Scan; this doc resolves it specifically by making the middle tier (Patrol) the Card and the top tier (Route Assignment) a separate rollup Card with no Feed children at all — sidestepping the need for transitive nesting via a rollup-field summary rather than solving the general case, since a live board has real clutter/performance constraints a one-time report doesn't. If a future module needs a genuine card→feed→feed structure where a true intermediate detail line (not just a summary stat) is needed at the middle tier *on this live board*, this doc's registration model needs a real extension, not assumed to just work. (For point-in-time reconstruction rather than a live board, unbounded depth is already solved — see [historical-cad-log-reconstruction.md](historical-cad-log-reconstruction.md)'s activity-axis walk, which reuses this doc's `parent_link_field` registrations without inheriting the one-level cap.)
