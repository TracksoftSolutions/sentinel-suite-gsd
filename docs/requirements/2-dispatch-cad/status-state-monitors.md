...
2. State transitions automatically, driven by the officer's **single active Dispatch**: a Dispatch entering the active range (assignment under `single_active`, or explicit queue advance under `queued` — see Unit Dispatch & Proximity Routing's Dispatch Stacking Policy) sets `dispatched`; that Dispatch's own phase logging (`en_route`, `arrived`/`on_scene`, `cleared`) drives the matching Unit State transition (`arrived` maps to `on_scene`; `cleared` maps to `completed`) in lockstep, no separate logging action required. This is well-defined by construction: the stacking policy's invariant guarantees at most one active Dispatch per officer at any moment, so there is never a "which Dispatch drives the state" ambiguity — `queued` Dispatches drive nothing, and a `preempted` Dispatch stops driving the instant its displacer becomes active.
3. Advancing out of `completed` is governed by a tenant-configurable **Completed State Policy**: `manual` (an officer or Supervisor must explicitly transition to `available` or `out_of_service` — no auto-advance), `auto` (advances to `available` immediately once the driving Dispatch clears), or `timer` (advances to `available` automatically after a configurable buffer duration, unless the officer/Supervisor explicitly intervenes first, e.g. to go `out_of_service` instead). A manual or not-yet-elapsed-timer transition to `out_of_service` is always available regardless of policy mode. **An explicit queue advance (Unit Dispatch's `queued` policy) is also always permitted from `completed`, regardless of policy mode** — taking the next queued call temporarily cuts the report-writing buffer short by deliberate human action; the policy governs passive return-to-available, never blocks an explicit advance.
...
**Unit State:** `available` ⇄ `out_of_service` (manual, either direction) | `available` → `dispatched` (new Dispatch assignment; from `out_of_service` requires confirmation-gate override) → `en_route` → `on_scene` → `completed` (all auto, mirroring the single active Dispatch's own phase) → `available` (explicit action under `manual` policy; automatic under `auto`/`timer` policy, per the tenant's Completed State Policy) | `out_of_service` (always available as an explicit action from `completed`, regardless of policy mode) | `completed` → `dispatched` (explicit queue advance under the `queued` stacking policy — permitted regardless of Completed State Policy mode, since it is a deliberate human action).
...
- ~~Exact precedence when an on-duty officer has more than one concurrently open Dispatch~~ — resolved: Unit Dispatch & Proximity Routing's Dispatch Stacking Policy (`single_active` | `queued`) guarantees at most one *active* Dispatch per officer under either mode; Unit State is driven exclusively by that single active Dispatch, and the "whichever most recently changed phase" shrug is retired.
