...
**Availability is a minimal, derived signal, not a full status system.** An on-duty officer is "available" for proximity suggestion if they have no open (non-cleared, non-cancelled) Dispatch already assigned to them. This doc does not attempt richer real-time status (on break, out of service, at a training) — that's explicitly deferred to **Status & State Monitors** (later in this module), which owns the fuller unit-status model this doc's minimal derivation will eventually plug into.
...
8. **Availability** (for suggestion purposes only) is derived, not stored: an on-duty officer is available if they have no Dispatch currently in an open phase (`dispatched` through `on_scene`, not yet `cleared` or `cancelled`) — a lightweight signal, not a full status system (see Overview; richer status states are Status & State Monitors' scope).
9. A Dispatcher may still manually assign an officer the system doesn't list as available (e.g., a supervisor judgment call to pull someone off a lower-priority task) — availability filtering narrows the default suggestion list, it never hard-blocks an override.
...
- **Status & State Monitors (Module 2, future)**: intended eventual owner of the fuller on-duty unit status model (available, en route, on break, out of service, etc.); this doc's derived availability signal (open-Dispatch-or-not) is the minimal precursor it will build on, not a competing mechanism.
...
- Full unit-status model (on break, out of service, training, etc.) — entirely deferred to Status & State Monitors, later in this module's elicitation queue.
