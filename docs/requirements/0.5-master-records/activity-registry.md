...
### High-frequency signals & the promotion valve
15. **An Activity is an operational record, not a union table.** The Activity base type exists for what a Dispatcher or Supervisor needs to see and act on efficiently — per explicit user framing during the platform design review — not to make every machine-generated signal an Entity row. High-frequency external signals (intrusion/fire panel events, IoT sensor readings, access-control events, device heartbeats — arriving via Module 19's future upstream-system adaptors, per the integrate-don't-replace boundary in `_DECISIONS.md`; this platform never speaks native device protocols) do **not** become Activities by default, extending the precedent already set twice (GIS Position Records; Entity Relationships & History keeping access events as raw telemetry).
16. **Signal Disposition is a tenant-configured spectrum, set per integration/signal type** (the configuration surface itself is owned by Module 19's future Physical Security Integration Gateway docs; this doc owns the Activity-side contract). Four levels:
    - `display_only` — the signal passes through to live consoles (wallboard, feed panels) and is not stored in this platform at all; the source system owns its own log.
    - `telemetry` — stored in a raw, retention-windowed telemetry store, outside the Entity hierarchy; queryable, but no Activity/queue/timeline presence. **Telemetry is the analytical substrate, not a discard pile**: each stored signal retains the full upstream metadata the integration exposes (precise timestamp, device/zone identity, signal/condition codes, raw payload where available) — most reporting software drops exactly these fields, and keeping them is what makes nuisance-alarm pattern analysis possible (e.g., an alarm recurring at exactly 05:45 on weekdays but 08:00 on weekends is an HVAC activation schedule wearing an intrusion alarm's costume — findable only from per-signal timing fidelity). Metadata richness honestly varies per upstream adaptor ("to the extent your integrations allow"); each adaptor declares what it captures rather than the platform pretending uniform depth.
    - `activity` — promotes to a real Activity extension (e.g., Alarm), with storm-collapse (#17), getting full standard treatment: queue eligibility, timeline inclusion, DAR pickup, Duration Watchdog eligibility.
    - `incident_dispatch` — promotes as above **and** triggers configured automation via ordinary Domain Events rules (auto-create an Incident via the launch-point mechanism, auto-dispatch responders — e.g., a forced door on a SCIF creates an Incident and dispatches armed guards). No new automation mechanism: this is the established trigger/effect split, subject to Domain Events' existing constraints (automation runs under the rule author's permissions and cannot target step-up-required actions).
...
- [ ] A signal type configured `display_only` appears on a live console and leaves zero rows in telemetry or the Entity hierarchy; the same signal reconfigured to `telemetry` is queryable in the telemetry store and still creates no Activity.
- [ ] A signal type configured `activity` firing 400 times from one source within the collapse window produces exactly one Activity with `occurrence_count` = 400 and correct first/last signal timestamps, while the raw sequence remains queryable in telemetry.
- [ ] A signal type configured `incident_dispatch` promotes the Activity and fires its configured Domain Events rule (Incident creation, dispatch) with the standard automation constraints applied.
- [ ] A Dispatcher can manually promote a `telemetry`-level signal into an Activity, and launch an Incident from it with context pre-filled, regardless of the signal type's configured disposition.
...
- Raw telemetry store retention defaults and physical shape (time-series table vs. log-structured store), and default collapse-window durations per signal family — technical-spec-level, revisited when Module 19 is specified; this doc fixes only the disposition contract and the collapse semantics.
- Whether a mid-storm disposition change (e.g., a Dispatcher escalating a collapsing alarm to `incident_dispatch` while signals are still arriving) splits the collapsed Activity or carries the whole storm into the escalation — deferred to Module 19's elicitation.
