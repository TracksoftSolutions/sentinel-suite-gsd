...
**Retrofit: a single, bounded `extended_fields` JSONB column on both TPT roots (`Entity` and `EntityAssociation`), generalized from a pattern first drafted locally in Courtesy Patrol.** That doc needed a small, tenant-configurable set of extra fields per courtesy category (a Jump Start cares about battery type; a Tire Change doesn't) and originally gave itself a bespoke `category_details` JSONB column to hold them. Promoting the storage mechanism here — available to *any* Entity or EntityAssociation type, at any TPT level — means every future feature with the same "a few optional extra fields that don't earn a full column/table" need reuses one audited, governed mechanism instead of reinventing it per feature. **This does not weaken the anti-EAV commitment above; it's a narrow, explicit escape hatch, not a second field-addition mechanism competing with it:**
- **Concrete columns on TPT subtype tables remain the default and expected way to add fields, full stop.** `extended_fields` exists only for the minority case where a full column/table is disproportionate to the field's value (a rarely-used, category-specific detail) — or for genuine **tenant-defined custom fields**, where a Tenant Admin needs to extend an existing type without a platform code change (a real, distinct use case from a feature developer's own field additions).
- It's **schema-governed, not truly free-form**: whatever declares that a given record should populate `extended_fields` (a type's own registration, item #26 below, or a consuming feature's own finer-grained selector, like Courtesy Patrol's per-category schema) declares a bounded field list (key, label, type, required) that values are validated against at write time.
- It carries an explicit, documented performance/discoverability tradeoff (see Non-Functional) — content in `extended_fields` is not guaranteed indexed or queryable the way a concrete column is, which is precisely why it isn't the default.
...
### Extended Fields (bounded escape hatch — not the default way to add fields)
...
30. **Concrete columns on TPT subtype tables remain the default and expected mechanism for adding fields.** `extended_fields` is reserved for two narrower cases: (a) a small set of category- or context-specific optional fields where a dedicated column or full TPT level would be disproportionate to the field's value (e.g., Courtesy Patrol's per-category detail fields), or (b) genuine tenant-defined custom fields, where a Tenant Admin extends an existing type for their own organization's needs without a platform code change.
31. `extended_fields` is **schema-governed, not free-form**. Whatever declares that a given record populates it — a type's own Entity/Extension Type Registration (`extended_fields_schema[]`, item #98 below) as a type-wide default, or a consuming feature's own finer-grained selector (e.g., Courtesy Patrol choosing a schema per tenant-configurable category, not per TPT level) — declares a bounded field list (key, label, type, required); values are validated against whichever schema applies at write time. A record with no applicable schema simply doesn't populate the field.
32. `extended_fields` content is not guaranteed indexed or queryable with the performance of a concrete column (see Non-Functional) — this is an explicit, accepted tradeoff of using the escape hatch instead of a real column, and part of why it's never the default choice.
...
- extended_fields (JSONB, nullable — bounded escape hatch, schema-governed, see Extended Fields above; not the default field-addition mechanism)
...
- extended_fields_schema[] (nullable — this type's own default schema for `extended_fields`: field_key, label, field_type, required; a consuming feature may instead select a finer-grained schema of its own, e.g. per tenant-configurable category, still writing to the same `extended_fields` column)
...
- extra_fields_schema_ref (pointer to where this kind's own TPT-level fields are defined, if any)
- extended_fields_schema[] (nullable — same shape and purpose as Entity Type Registration's, applied to this association kind's rows)
...
- extended_fields (JSONB, nullable — same bounded, schema-governed escape hatch as Entity's, see Extended Fields above)
...
- **Courtesy Patrol** (Security Operations): first consumer of `extended_fields` — its per-category detail fields (Jump Start battery type, Tire Change tire position) are stored here rather than in a feature-local JSONB column, with Courtesy Patrol's own Category Definition supplying the finer-grained (per-category, not per-TPT-level) schema selection.
- **Settings & Preferences**: a plausible future home for a Tenant Admin-facing UI over tenant-defined `extended_fields_schema` entries, letting tenants add their own custom fields to an existing type without a platform change — not built out here, this doc only owns the storage/validation mechanism.
...
- `extended_fields` (JSONB) is not guaranteed indexed or queryable with concrete-column performance — any feature relying on it for a field that needs to be efficiently filtered/sorted/reported on at scale has chosen the wrong mechanism and should use a concrete TPT column instead; this tradeoff must be documented wherever `extended_fields` is consumed, not discovered later.
- Writes to `extended_fields` must be validated against whichever schema applies (type-level default or a consuming feature's finer-grained selector) at write time — an unrecognized key or a missing `required` key is rejected, but a record with no applicable schema at all is never blocked from saving.
...
- [ ] A Courtesy Patrol record's category-specific detail fields are correctly stored in and read from `extended_fields` on its underlying Entity row, validated against the schema its Category Definition selects.
- [ ] Writing an `extended_fields` value with a key not present in the applicable schema is rejected; a missing `required` key is rejected; a missing non-required key succeeds.
- [ ] A record whose type has no `extended_fields_schema` declared at all, and no consuming feature supplying one, can still be saved without populating `extended_fields`.
...
- Whether a full Tenant Admin-facing UI for self-service `extended_fields_schema` definition (true tenant-custom-fields, likely living in Settings & Preferences) gets built now or deferred — this doc only specifies the underlying storage/validation mechanism, not that administrative surface.
- Exact `field_type` vocabulary for `extended_fields_schema` (text/number/boolean/select, and whether `select` needs its own option-list sub-structure) — a technical-spec-level decision, same open question already raised locally in Courtesy Patrol.
- Whether `extended_fields` ever needs partial/JSONB-path indexing for a specific high-value consuming feature, as a targeted exception to the general "not guaranteed queryable" tradeoff — not anticipated now, revisit if a real case emerges.
